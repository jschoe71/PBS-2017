...
We have developed an integrated polarization beam splitter (PBS) module with silica planar lightwave circuit technology for use in BB84 quantum key distribution.
% 4개의 편광에 대해 동작시키기 위해 반파장판을 삽입하였으며, 모듈의 편광소광비와 삽입손실은 각각 ___dB와 ___dB였다.
...
%The excess optical loss and The minimum polarization extinction ratio were \underline{4.59}dB and \underline{17.6dB}, respectively.
% 무선양자암호 테스트베드에 장착하였을 때 QBER과 sifted key rate은 까각 ___%와 ___kbps롤 보였다.
When the module was loaded on the free-space BB84 test-bed, quantum bit error rate and sifted key rate  were 2.81\% and 415 kbps, respectively.
\end{abstract}
...
Many researches have been conducted for quantum communication as security became one of the main issues in communication technology \cite{Bennett:1984is, Ekert:1991kl}.
...
Implementation of BB84 requires two sets of non-orthogonal bases %that can be constructed with either polarization or phase.
...
The polarization-based BB84 encodes the data by choosing a basis of the two bases of horizontal(H)-vertical(V) or diagonal(D)-antidiagonal(A) polarization, and choosing one polarization from the basis to generate an encryption key.
...
Thus an optical part for splitting or combining polarizations is essential for a polarization-based BB84 QKD, and is generally composed of two bulk polarization beam splitters (PBS), one beam splitter, and one half-wave plate (HWP) \cite{Ko:2017cs}.
...
PBS developed in this work is based on Mach-Zehnder interferometer (MZI) fabricated with silica PLC technology \cite{Kim:2012ej, Hashizume:2015ta}.
% 사용하는 파장 대역이 780 nm이므로 여기에서 단일 모드가 되며 해당 파장에서의 광섬유와 높은 광결합 효율을 가지도록 기본 도파로를 설계하였다.
The waveguide structure was chosen to support single mode and maximum mode matching with single mode fiber (SMF) at the working wavelength of 780 nm, where Si single photon detector works and atmospheric absorption is low.
...
% If the transmission axis of the LP is vertical, only V-polarized light is considered to be incident since the LP filters out the H-polarized light with extinction ratio exceeding 30dB.
...
% 50 um 두께의 블레이드로 공정하였을 때 실제로 형성된 홈의 폭은 약 60 um 정도였고, 이 간격과 chipping에 의해 발생하는 광손실은 ___dB였다.
...
% 에폭시를 주입하였을 때 chipping의 효과를 줄이고 빛의 diverging도 줄이므로 손실은 ___dB 정도로 감소하였다.
...
% 이론적으로 홈의 폭을 30 um 로 줄이면 손실은 ___dB 정도로 줄어들므로 얇은 반파장판의 제작이 전체적인 특성 개선을 가져올 것으로 기대된다.
%Simulation predicts that the reduced width of the slit to 30 $\mu$m will reduce the optical loss by \underline{1.5}dB.
%Thus using thinner waveplate film and blade will help improve optical loss by about 1.5dB.
...
% 그것들 중 홈에서의 손실이 ___dB로 추정되며 이것은 더 좁은 홈과 거기에 맞는 반파장판을 사용하면 줄일 수 있다.
% 30um로 홈을 줄이면 1.5dB 정도의 손실을 줄일 수 있을 것으로 예상된다.
...
\begin{figure}[ht]
  ...
  \subfigure[]{\includegraphics[height=5.2cm]{QBER}}
...
% ___dB의 손실은 이상적으로는 ___%의 QBER을 유발한다.
...
% CW 레이저를 이용하여 측정한 편광 소광비는 최소 ___dB를 보였고 제작된 모듈을 무선 양자암호 테스트베드에 장착하여 테스트 한 결과 sifted key rate는 ___kbps, QBER은 ___% 였으므로 보안 통신을 할 수 있는 충분한 성능을 확인하였다.
